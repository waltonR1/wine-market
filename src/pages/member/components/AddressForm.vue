<template>
  <view class="fixed inset-0 z-50">
    <view class="absolute inset-0 bg-black/40" @click="$emit('close')" />

    <view class="absolute left-0 right-0 bottom-0 bg-white rounded-t-3xl px-4 pt-4 pb-6 safe-area-inset-bottom">
      ...
      <view class="space-y-3">
        <input
          v-model="form.name"
          placeholder="收货人"
          class="bg-[#F5F5F5] rounded-lg px-3 py-2 text-[13px] mb-1"
        />
        ...
        <picker mode="region" @change="handleRegionChange">
          <view class="bg-[#F5F5F5] rounded-lg px-3 py-2 text-[13px] mb-1">
        ...
        <input
          v-model="form.detail"
          placeholder="详细地址"
          class="bg-[#F5F5F5] rounded-lg px-3 py-2 text-[13px]"
        />
        ...
        <view v-if="form.isDefault" class="text-[11px] text-[#999] mt-1">
          设为默认后，将自动替换当前默认地址
        </view>
      </view>
      ...
      <view class="mt-5 flex gap-3">
        <view
          class="flex-1 bg-[#F5F5F5] text-text-main text-center py-3 rounded-full text-[14px]"
          @click="$emit('close')"
        >
          取消
        </view>
        <view
          class="flex-1 bg-accent text-white text-center py-3 rounded-full text-[14px] font-medium"
          @click="$emit('submit')"
        >
          保存
        </view>
